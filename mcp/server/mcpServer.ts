...
import { MCPMessage, MCPMessageType, MCPValidationResult } from '../types/mcpTypes';
...
interface MCPServerEvents {
  ...
  stateUpdated: (context: ProjectContext) => void;
  actionLogged: (action: any) => void;
}
...
export class MCPServer extends EventEmitter {
  ...
  private async handleComponentAdded(payload: ActionPayloads['COMPONENT_ADDED']): Promise<void> {
    const validationResult = await validateProjectStructure();
    if (!validationResult.isValid) {
      throw new Error(`Project structure validation failed: ${validationResult.errors?.join(', ')}`);
    }
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      this.server.on('connection', (socket: Socket) => {
        ...
        console.log(`[INFO] New client connected: ${clientId}`);
        ...
        socket.on('close', () => {
          console.log(`[INFO] Client disconnected: ${clientId}`);
          this.clients.delete(clientId);
        });

        socket.on('error', (error) => {
          console.error(`[ERROR] Socket error for client ${clientId}:`, error);
          this.clients.delete(clientId);
        });
      });
    } catch (error) {
  ...
  private async handleMessage(message: MCPMessage, socket: Socket): Promise<void> {
    try {
      switch (message.type) {
        case MCPMessageType.STATE_UPDATE:
          await this.handleStateUpdate(message.payload);
          break;
        case MCPMessageType.ACTION_LOG:
          await this.handleActionLog(message.payload);
          break;
        default:
  ...
  private async handleStateUpdate(payload: any): Promise<void> {
    ...
    this.emit('stateUpdated', this.context);
  }

  private async handleActionLog(payload: any): Promise<void> {
    this.actionLog.push(payload);
    this.emit('actionLogged', payload);
  }

  public stop(): void {
    this.server.close();
    this.clients.forEach((socket) => socket.end());
    this.clients.clear();
  }
}
